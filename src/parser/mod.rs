...
fn cmake_statement(input: &str) -> IResult<&str, CMakeStatement> {
    alt((
        cmake_command.map(|item| CMakeStatement::CMakeCommandStatement(item)),
        cmake_comment.map(|item| CMakeStatement::CMakeCommentStatement(item.to_string())),
        tuple((tag("\n"), space0)).map(|_| CMakeStatement::CMakeNewline),
    ))(input)
}
